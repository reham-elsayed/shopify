...
document.addEventListener('click', function (event) {
  ...
  if (productCard && !event.target.closest('a, button, input, textarea, select, [role="button"][aria-expanded]')) {
...
document.addEventListener('keydown', function (event) {
  if (event.key === 'Enter' || event.key === ' ') {
    const productCard = event.target.closest('[data-product-url][role="button"]');
    if (productCard) {
      event.preventDefault();
      window.location.href = productCard.dataset.productUrl;
    }
